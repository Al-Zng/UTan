...
struct SubtitleSettingsView: View {
    ...
    // نحفظ قيمة التأخير عند فتح الشاشة لنرجعها عند "تطبيق على الحلقة فقط"
    @State private var originalDelay: Double = AppSettings.shared.subtitleDelay
...
struct CustomPlayerView: View {
                ...
                VStack(spacing: 0) {
                    ...
                    HStack(spacing: 52) {
                        Button {
                            ...
                            let t   = max(0, now - 10)
                            player.seek(to: CMTime(seconds: t, preferredTimescale: 600))
                            withAnimation { seekFeedback = (show: true, isRight: false) }
                        ...
                        Button {
                            let now = player.currentTime().seconds
                            let dur = player.currentItem?.duration.seconds ?? duration
                            let t   = min(dur.isNaN ? duration : dur, now + 10)
                            player.seek(to: CMTime(seconds: t, preferredTimescale: 600))
                            withAnimation { seekFeedback = (show: true, isRight: true) }
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                                withAnimation { seekFeedback.show = false }
                            }
                            scheduleHide()
    ...
    private func switchToEpisode(_ ep: EpisodeItem, autoplay: Bool = true) {
        ...
        // إذا كان التأخير "مؤقتاً" لحلقة سابقة، نُعيد الأصلي المحفوظ
        let savedDefault = UserDefaults.standard.double(forKey: "sub_delay_saved_default")
        if settings.subtitleDelay != savedDefault {
            settings.subtitleDelay = savedDefault
        }
        ...
        ZStack {
            if showLoader {
                ...
                .onAppear {
                    ...
                    tabAppearance.backgroundColor = UIColor.black.withAlphaComponent(0.85)
                        ...
                        ScrollView(showsIndicators: false) {
                            LazyVStack(spacing: 0) {
                                ...
                                VStack(alignment: .leading, spacing: 30) {
                                    ...
                                    // Trending Today — من heroItems (الكاروسيل/Featured)
                                    if scraper.heroItems.count >= 5 {
                                        Top10Row(title: L("الأكثر مشاهدة اليوم", "Trending Today"), items: Array(scraper.heroItems.prefix(10)))
                                    } else if let firstCat = scraper.categories.first, firstCat.items.count >= 5 {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(alignment: .bottom, spacing: 8) {
                    ForEach(Array(items.prefix(10).enumerated()), id: \.element.id) { rank, item in
                        NavigationLink(destination: LazyDestination(DetailsView(itemId: item.id))) {
                            ZStack(alignment: .bottomLeading) {
                                // Poster — fully contained, no overflow
                                CachedAsyncImage(url: URL(string: item.imageUrl)) { phase in
                                    if let image = phase.image {
                                        image.resizable().aspectRatio(contentMode: .fill)
                                ...
                                // Bottom gradient — readability for the number
                                LinearGradient(
                                ...
                                // Rank number — clipped INSIDE card bounds, no overflow into neighbor
                                Text("\(rank + 1)")
                            ...
                            .frame(width: 110, height: 165)
                            .clipped()                       // hard-clip — number never bleeds out
                            .contentShape(Rectangle())        // tap area == exact card frame
                        }
                        .buttonStyle(ScaleButtonStyle())
...
struct CategoryListView: View {
    ...
    var body: some View {
        ZStack {
            APP_BG.ignoresSafeArea()
            ScrollView {
                // شريط الترتيب — Apple TV minimal style
                HStack {
                    ...
                    Button { showGenrePicker = true } label: {
                        Image(systemName: "line.3.horizontal.decrease")
                            .foregroundColor(selectedGenre.isEmpty ? .white.opacity(0.7) : UT_RED)
                            .padding(8)
                            ...
                            if d.isMovie {
                                Button { playMovie(d: d) } label: {
                                    HStack(spacing: 7) {
                                        ...
                                        Text(L("تشغيل", "Play")).font(.system(size: 15, weight: .semibold)).minimumScaleFactor(0.8).lineLimit(1)
                                    }
                                    .foregroundColor(.black)
                                ...
                                Button { playEpisode(d: d, ep: first) } label: {
                                    HStack(spacing: 7) {
                                        Image(systemName: "play.fill").font(.system(size: 14, weight: .bold))
                                        Text(L("تشغيل الحلقة الأولى", "Play First Episode")).font(.system(size: 15, weight: .semibold)).minimumScaleFactor(0.8).lineLimit(1)
                                    }
                                    .foregroundColor(.black)
                                    .frame(maxWidth: .infinity).frame(height: 46)
                                    .background(Color.white)
